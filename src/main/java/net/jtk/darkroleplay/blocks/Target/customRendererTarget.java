...
public class customRendererTarget extends TileEntitySpecialRenderer {
	private static final ResourceLocation texture = new ResourceLocation(DarkRoleplay.MODID, "textures/blockTarget.png");

	private ModelTarget model;

	public customRendererTarget() {
		this.model = new ModelTarget();
	}
	
	@Override
	public void renderTileEntityAt(TileEntity tileentity, double x, double y,double z, float f, int i) {
		if(tileentity.getWorld().getBlockState(tileentity.getPos()).getBlock().equals(Target.blockTarget)){
			GL11.glPushMatrix();
