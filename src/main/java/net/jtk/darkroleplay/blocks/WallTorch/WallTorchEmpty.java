...
public class WallTorchEmpty extends BlockContainer{
	...
	public WallTorchEmpty() {
		super(Material.wood);
		this.setDefaultState(this.blockState.getBaseState().withProperty(DIR, Integer.valueOf(0)).withProperty(AddonLighter, false).withProperty(AddonTrap, false));
	}
	...
	public IBlockState getStateFromMeta(int meta)
    {
		int i = meta;
		if(i < 4){
			return this.getDefaultState().withProperty(DIR, Integer.valueOf(meta)).withProperty(AddonLighter, false).withProperty(AddonTrap, false);
		}else if(i >= 4 && i < 8){
			return this.getDefaultState().withProperty(DIR, Integer.valueOf(meta-4)).withProperty(AddonLighter, true).withProperty(AddonTrap,false);
		}else if(i >= 8 && i < 12){
			return this.getDefaultState().withProperty(DIR, Integer.valueOf(meta-8)).withProperty(AddonLighter, false).withProperty(AddonTrap,true);
		}
	    ...
	    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
	    {
	    	if(!worldIn.isRemote){
	    		if(playerIn.getHeldItem() != null){
	    			...
	    			worldIn.setBlockState(pos, WallTorchUnlit.blockWallTorchUnlit.getDefaultState().withProperty(DIR, state.getValue(DIR)).withProperty(AddonLighter, state.getValue(AddonLighter)).withProperty(AddonTrap, state.getValue(AddonLighter)));
