	    	...
	    	if(!worldIn.isRemote){
	    		if(playerIn.getHeldItem() == null){
	    			if((Boolean)state.getValue(AddonTrap) && !(Boolean)state.getValue(POWERED)){
			            ...
			            worldIn.scheduleUpdate(pos, WallTorchUnlit.blockWallTorchUnlit, 60);
	    ...
	    public void onNeighborBlockChange(World worldIn, BlockPos pos, IBlockState state, Block neighborBlock)
	    {
	    	...
	    	if((Boolean)state.getValue(AddonLighter)){
	    		 if (!worldIn.isRemote)
	 	        {
	 	            ...
	 	            {
	 	            	 worldIn.setBlockState(pos, WallTorchUnlit.blockWallTorchUnlit.getDefaultState().withProperty(DIR, state.getValue(DIR)).withProperty(AddonLighter, state.getValue(AddonLighter)).withProperty(AddonTrap, state.getValue(AddonTrap)).withProperty(POWERED, state.getValue(POWERED)), 2);
	 	            }
	 	        }
	    	}
	    }
